...
| `report_issue` | Report anything that did not work to the LangWatch team (no API key needed, user permission required, secrets redacted locally). See the [reporting guide](/support#reporting-issues-from-coding-agents) |
...
- `model` (required): Model name (e.g., `"gpt-5-mini"`, `"claude-sonnet-4-5-20250929"`)
