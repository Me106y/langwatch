...
1. **Create your experiment** in the [Experiments via UI](https://app.langwatch.ai/@project/experiments)
   - Add your dataset
...
```
  ...
  <Tab title="TypeScript">
```typescript
const { rows, runUrl } = await langwatch.experiments.runWithResults("my-experiment");
console.table(rows);        // per-row results
console.log(runUrl);        // open the run in LangWatch
```
  </Tab>
</Tabs>
...
```python
# Inline rows
langwatch.experiment.run("my-experiment", data=[{"input": "What is 2 + 2?"}])
# A platform dataset by id
langwatch.experiment.run("my-experiment", dataset_id="dataset_xxxxxxxxxxxx")
# Constant parameters merged into every row
langwatch.experiment.run("my-experiment", parameters={"model": "gpt-5-mini"})
```
  </Tab>
  <Tab title="TypeScript">
```typescript
// Inline rows
await langwatch.experiments.runWithResults("my-experiment", { data: [{ input: "What is 2 + 2?" }] });
// A platform dataset by id
await langwatch.experiments.runWithResults("my-experiment", { datasetId: "dataset_xxxxxxxxxxxx" });
// Constant parameters merged into every row
await langwatch.experiments.runWithResults("my-experiment", { parameters: { model: "gpt-5-mini" } });
```
  </Tab>
</Tabs>
...
for idx, row in experiment.loop(dataset.iterrows()):
    def compare(idx, row):
        ...
        with experiment.target("gpt-5-mini", {"model": "gpt-5-mini"}):
            response = call_openai("gpt-5-mini", row["input"])
        ...
        # Test the Anthropic model
        with experiment.target("claude-sonnet-5", {"model": "claude-sonnet-5"}):
            response = call_anthropic("claude-sonnet-5", row["input"])
            experiment.log_response(response)
            experiment.evaluate("ragas/faithfulness", index=idx, data={
                "input": row["input"],
                "output": response,
...
────────────────────────────────────────────────────────────
  TARGETS:
    gpt-5-mini: 20 passed, 2 failed
      Avg latency: 1250ms
      Total cost: $0.0125
    claude-sonnet-5: 22 passed, 1 failed
      Avg latency: 980ms
      Total cost: $0.0098
────────────────────────────────────────────────────────────
  EVALUATORS:
...
```bash
curl "https://app.langwatch.ai/api/evaluations/v3/runs/{runId}/results?experimentSlug={slug}" \
  -H "X-Auth-Token: ${LANGWATCH_API_KEY}"
```
