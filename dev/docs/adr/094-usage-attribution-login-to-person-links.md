...
| Erasure blanks | `userId` and `actorUserId` on link rows (and person references in `DiscoveredAgent.snapshot`); anchor `externalId` on `OrganizationUser`; email-kind `externalId` swapped for the keyed-hash token; `erasedAt` stamped on every link/inventory row whose **own subject** was erased — an actor-only touch is blanked without a stamp, and the mutable anchor gets no marker (Decision 9, scope narrowed v8 proposed) | rows, non-email login ids, `externalKind`, timestamps, org/connection keys survive — no timeline row ever disappears |
| Erasure blast radius | the values erasure tokenizes or scrubs come **only** from rows where the person is the SUBJECT (`userId`), never from rows they authored as an admin (`actorUserId`) | a row an admin merely typed carries a COLLEAGUE's login id; collecting from both would irreversibly hash third parties' logins whenever an admin is forgotten |
...
- **v8 — PROPOSED, captain sign-off pending at review of the read-path PR.** Decision 9's stamping rule contradicted Decision 9's stated purpose: "stamp every touched row" collides with `resolveOwnerAt`, which reads `userId` null plus `erasedAt` as "erased-person" and puts that login in the **attributed** bucket — so an unlink row the erased person merely authored would move a third party's spend out of "unattributed" and attribute it to somebody who never owned the login. Resolution: `erasedAt` marks rows whose OWN subject identity was erased; rows touched only to blank `actorUserId` are blanked without a stamp. Implemented in the read-path batch and pinned by a test; merging that PR is the sign-off. A second, unrelated clarification rides with it, drawn from the same review: the values erasure tokenizes come only from rows where the person is the subject, never from rows they authored — an admin's rows carry colleagues' login ids, and the keyed hash is one-way.

- **v7 — 2026-08-14 — Decision 9 amendment: Microsoft `upn` counts as an email-kind id; captain: Sergio Esteban.** The fork was asked, not silently re-decided: v6 said "only kinds literally named `email`" and flagged `upn` as an open spec question. Answer: `upn` is email-shaped and names the person, so erasure swaps it for the same org-scoped keyed-hash token — a `upn` surviving erasure would leave the person's name readable in link rows. `EMAIL_EXTERNAL_KINDS` in `@langwatch/identity-links` is now `["email", "upn"]`; everywhere this document says "email-kind", read "email or `upn`".

## Final ER model

Solid lines are stored id columns (plain ids the service validates — no schema-level relations, see the isolation invariant); dashed lines are resolved at report time, not stored.
